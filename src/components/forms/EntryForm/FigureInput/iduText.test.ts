...
const cases: Case[] = [
    ...
    { n: 3, input: { ...base, term: 'FORCED_TO_FLEE', unit: 'PERSON', reported: 3, quantifier: 'APPROXIMATELY', geoLocations: [geo('ORIGIN', 'Kharkiv, Ukraine'), geo('DESTINATION', 'Poltava, Ukraine')], ...conflict('13'), sources: gov2, startDate: '2025-12-01', endDate: '2025-12-16' }, expected: 'According to local authorities and national authorities, around three people were forced to flee from Kharkiv to Poltava due to communal violence between the 1st and 16th of December 2025.' },
    { n: 4, input: { ...base, term: 'RELOCATED', unit: 'HOUSEHOLD', reported: 12450, quantifier: 'LESS_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN_AND_DESTINATION', 'Huambo, Angola')], ...disaster('15'), sources: iom, startDate: '2024-11-06', endDate: '2025-02-01' }, expected: 'According to International Organization for Migration (IOM), up to 12,450 households were relocated in Huambo due to a landslide between November 6, 2024 and February 1, 2025.' },
    { n: 5, input: { ...base, term: 'SHELTERED', unit: 'PERSON', reported: 1, quantifier: 'EXACT', geoLocations: [geo('ORIGIN', 'Kathmandu, Bagmati, Nepal')], ...disaster('1'), sources: [], startDate: '2025-10-01' }, expected: 'According to (Source), one person was sheltered in Kathmandu due to an earthquake on October 1, 2025.' },
    { n: 6, input: { ...base, term: 'HOMELESS', unit: 'HOUSEHOLD', reported: 27, quantifier: 'MORE_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN', 'Poza Rica, Veracruz, Mexico')], ...disaster('13'), sources: gov, startDate: '2025-10-01', endDate: '2025-10-10' }, expected: 'According to national authorities, at least 27 households were rendered homeless in Poza Rica due to flooding between the 1st and 10th of October 2025.' },
    ...
    { n: 8, input: { ...base, term: 'IN_RELIEF_CAMP', unit: 'HOUSEHOLD', reported: 2, quantifier: 'LESS_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN', 'Cox Bazar, Bangladesh')], ...disaster('13'), sources: gov2, startDate: '2025-07-04' }, expected: 'According to local authorities and national authorities, up to two households were in a relief camp in Cox Bazar due to flooding on July 4, 2025.' },
    { n: 9, input: { ...base, term: 'RETURNS', unit: 'HOUSEHOLD', reported: 340, quantifier: 'MORE_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN', 'Kharkiv, Ukraine'), geo('DESTINATION', 'Donetsk, Ukraine')], ...conflict('2'), sources: gov, startDate: '2025-03-01', endDate: '2025-09-15' }, expected: 'According to national authorities, at least 340 households returned from Kharkiv to Donetsk following international armed conflict between March 1 and September 15, 2025.' },
    { n: 10, input: { ...base, term: 'RETURNS', unit: 'PERSON', reported: 1, quantifier: 'EXACT', geoLocations: [geo('ORIGIN', 'Aleppo, Syria')], ...disaster('13'), sources: media, startDate: '2025-05-01' }, expected: 'According to media sources, one person returned in Aleppo following flooding on May 1, 2025.' },
    { n: 11, input: { ...base, term: 'MULTIPLE_OR_OTHER', unit: 'PERSON', reported: 60, quantifier: 'APPROXIMATELY', geoLocations: [geo('ORIGIN_AND_DESTINATION', 'Nairobi, Kenya')], ...other('1'), sources: iom, startDate: '2025-08-01', endDate: '2025-08-20' }, expected: 'According to International Organization for Migration (IOM), around 60 people were displaced in Nairobi due to development between the 1st and 20th of August 2025.' },
    ...
    { n: 13, input: { ...base, term: 'DESTROYED_HOUSING', unit: 'PERSON', reported: 4, quantifier: 'MORE_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN', 'Kathmandu, Nepal')], ...disaster('1'), sources: [], startDate: '2025-04-25' }, expected: 'According to (Source), the housing of at least four people was destroyed in Kathmandu due to an earthquake on April 25, 2025.' },
    { n: 14, input: { ...base, term: 'PARTIALLY_DESTROYED_HOUSING', unit: 'HOUSEHOLD', reported: 9, quantifier: 'EXACT', geoLocations: [geo('ORIGIN', 'Beira, Mozambique')], ...disaster('13'), sources: gov2, startDate: '2025-03-01', endDate: '2025-03-14' }, expected: 'According to local authorities and national authorities, nine houses were partially destroyed in Beira due to flooding between the 1st and 14th of March 2025.' },
    { n: 15, input: { ...base, term: 'PARTIALLY_DESTROYED_HOUSING', unit: 'PERSON', reported: 210, quantifier: 'APPROXIMATELY', geoLocations: [geo('ORIGIN', 'Freetown, Sierra Leone')], ...disaster('15'), sources: media, startDate: '2024-08-14', endDate: '2025-01-10' }, expected: 'According to media sources, the housing of around 210 people was partially destroyed in Freetown due to a landslide between August 14, 2024 and January 10, 2025.' },
    { n: 16, input: { ...base, term: 'UNINHABITABLE_HOUSING', unit: 'HOUSEHOLD', reported: 1, quantifier: 'LESS_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN', 'Tacloban, Philippines')], ...disaster('13'), sources: gov, startDate: '2025-09-09' }, expected: 'According to national authorities, one house was rendered uninhabitable in Tacloban due to flooding on September 9, 2025.' },
    { n: 17, input: { ...base, term: 'UNINHABITABLE_HOUSING', unit: 'PERSON', reported: 33, quantifier: 'MORE_THAN_OR_EQUAL', geoLocations: [geo('ORIGIN_AND_DESTINATION', 'Manila, Philippines')], ...other('2'), sources: iom, startDate: '2025-06-01', endDate: '2025-06-30' }, expected: 'According to International Organization for Migration (IOM), the housing of at least 33 people was rendered uninhabitable in Manila due to eviction between the 1st and 30th of June 2025.' },
    ...
    { n: 23, input: { ...base, term: 'AFFECTED', unit: 'PERSON', reported: 7, quantifier: 'APPROXIMATELY', geoLocations: [geo('ORIGIN', 'Chennai, Tamil Nadu, India')], ...disaster('7'), sources: gov2, startDate: '2025-11-11', endDate: '2025-11-13' }, expected: 'According to local authorities and national authorities, around seven people were affected in Chennai due to a drought between the 11th and 13th of November 2025.' },
...
describe('generateExcerptIduText (branch edges)', () => {
    ...
    it('unmapped disaster subtype -> placeholder, no crash', () => {
        expect(generateExcerptIduText({ ...base, figureCause: 'DISASTER', disasterSubType: '999', reported: 5, unit: 'PERSON', term: 'DISPLACED', sources: gov }))
            .toContain('due to (Main trigger)');
    });
    it('unmapped conflict subtype -> placeholder, no crash', () => {
        expect(generateExcerptIduText({ ...base, figureCause: 'CONFLICT', violenceSubType: '999', reported: 5, unit: 'PERSON', term: 'DISPLACED', sources: gov }))
            .toContain('due to (Main trigger)');
    });
    it('unmapped other subtype -> placeholder, no crash', () => {
        expect(generateExcerptIduText({ ...base, figureCause: 'OTHER', otherSubType: '999', reported: 5, unit: 'PERSON', term: 'DISPLACED', sources: gov }))
            .toContain('due to (Main trigger)');
    });
    ...
    it('lone Local Authority source (full sentence)', () => {
...
describe('array cardinality (1 and 3 items)', () => {
    ...
    it('origin equals destination -> in', () => {
        expect(generateExcerptIduText({ ...frame, geoLocations: [geo('ORIGIN', 'Kathmandu, Nepal'), geo('DESTINATION', 'Kathmandu, Nepal')] }))
            .toBe(line('in Kathmandu'));
    });
    it('origin set equals destination set regardless of order -> in', () => {
    ...
    it('3 origin-and-destinations', () => {
        expect(generateExcerptIduText({ ...frame, geoLocations: [geo('ORIGIN_AND_DESTINATION', 'Kyiv, Ukraine'), geo('ORIGIN_AND_DESTINATION', 'Lviv, Ukraine'), geo('ORIGIN_AND_DESTINATION', 'Odesa, Ukraine')] }))
            .toBe(line('in Kyiv, Lviv, and Odesa'));
    });
    it('1 source', () => {
...
describe('formatSource', () => {
    it('empty -> (Source)', () => expect(formatSource([])).toBe('(Source)'));
    it('single authority -> national', () => expect(formatSource(gov)).toBe('national authorities'));
    it('lone Local Authority -> local authorities', () => expect(formatSource([{ name: 'City Hall', organizationKind: { name: 'Local Authority' } }])).toBe('local authorities'));
    it('government + local authority -> both', () => expect(formatSource(gov2)).toBe('local authorities and national authorities'));
    ...
    it('non-empty but no usable name/kind -> (Source)', () => expect(formatSource([{ organizationKind: { name: 'Academia' } }])).toBe('(Source)'));
});
