...
function useChartData<DATUM>(
    ...
    const renderableWidth = chartSize.width
        - chartOffset.left
        - chartMargin.left
        - chartPadding.left;
    const renderableHeight = chartSize.height
        - chartOffset.bottom
        - chartPadding.bottom
        - chartMargin.bottom;

    const xAxisTickWidth = renderableWidth / (numXAxisTicks + 2);
    ...
    const xScaleFn = useMemo(
        () => getScaleFunction(
            ...
            {
                start: chartOffset.left
                    + chartMargin.left
                    + chartPadding.left
                    + xAxisTickWidth / 2,
                end: chartOffset.right
                    + chartMargin.right
                    + chartPadding.right
                    + xAxisTickWidth / 2,
            },
        ),
        [xDataBounds, chartOffset, chartMargin, chartSize, chartPadding, xAxisTickWidth],
    );
        ...
        () => {
            if (type === 'numeric') {
                ...
                return intervals.map(
                    (tick) => ({
                        key: tick,
                        x: xScaleFn(tick),
                        label: xAxisLabelSelector(tick as number & DATUM),
                    }),
                );
            }
                ...
                return intervals.map(
                    (tick) => {
                        ...
                        return {
                            ...
                            label: xAxisLabelSelector(timestamp as number & DATUM),
                        };
                    },
                );
            }
            ...
            return dataPoints.map(
                (dataPoint) => ({
                    key: dataPoint.key,
                    x: dataPoint.x,
                    label: xAxisLabelSelector(dataPoint.originalData as number & DATUM),
                }),
            );
        },
